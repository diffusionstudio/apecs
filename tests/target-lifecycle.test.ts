...
describe("onTargetDespawn: 'despawn' (§7.5)", () => {
  test('exclusive: the whole subtree goes down with the target', () => {
    ...
    for (const e of [root, a, b, c]) expect(world.isAlive(e)).toBe(false)
  ...
  test('a deep chain cascades without growing the stack', () => {
    ...
    const root = previous
    for (let i = 0; i < depth; i++) previous = world.spawn(PartOf(previous))
  ...
  test('a wide tree cascades through every level', () => {
    ...
    for (let d = 0; d < 4; d++) {
      ...
      for (const parent of level)
        for (let i = 0; i < 3; i++) next.push(world.spawn(Position, PartOf(parent)))
      total += next.length
      level = next
    }
  ...
  test('a cycle terminates cleanly', () => {
    ...
    for (const e of [a, b, c, x, y]) expect(world.isAlive(e)).toBe(false)
  ...
  test('despawnMany over a query of sources cascades once each', () => {
    const world = new World()
    const root = world.spawn()
    for (let i = 0; i < 3; i++) world.spawn(PartOf(world.spawn(PartOf(root))))
