...
describe('representation (§4.1)', () => {
  ...
  test('handles are unique across worlds', () => {
    ...
    for (const world of [a, b]) for (const entity of world.spawnMany(100)) seen.add(entity)
...
describe('lifecycle (§4.2)', () => {
  ...
  test('a recycled id does not resurrect the old handle', () => {
    ...
    const fresh: Entity[] = []
    for (let i = 0; i < 64; i++) fresh.push(world.spawn(Position))

    expect(world.isAlive(stale)).toBe(false)
    for (const entity of fresh) expect(entity).not.toBe(stale)
  })
...
describe('bulk operations (§4.3)', () => {
  test('spawnMany returns a Float64Array of n live handles', () => {
    ...
    for (const entity of swarm) expect(world.isAlive(entity as Entity)).toBe(true)
  ...
  test('despawnMany accepts a batch', () => {
    ...
    expect(world.query(Position).count).toBe(0)
    for (const entity of swarm) expect(world.isAlive(entity as Entity)).toBe(false)
  })
  ...
  test('a batch and the loop it replaces produce the same world', () => {
    ...
    for (let i = 0; i < 100; i++) loop.spawn(Position({ x: 2 }))

    bulk.addMany(swarm, IsActive)
    for (const entity of loop.query(Position).entities()) loop.add(entity as Entity, IsActive)
  ...
  test('spawning across a page boundary keeps every value intact', () => {
    ...
    for (let i = 0; i < swarm.length; i++) world.set(swarm[i] as Entity, Position.x, i)
    for (let i = 0; i < swarm.length; i++) expect(world.get(swarm[i] as Entity, Position.x)).toBe(i)
  })
})
