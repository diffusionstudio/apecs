...
describe('spawnMany (§4.3)', () => {
  test('returns a Float64Array of live, distinct handles', () => {
    ...
    for (const e of handles(batch)) expect(world.isAlive(e)).toBe(true)
  ...
  test('the whole batch lands in one archetype at contiguous rows', () => {
    ...
    for (const e of batch) expect(archetypeOf(world, e)).toBe(archetype)
    for (let row = 0; row < 20; row++) expect(archetype.entityAt(row)).toBe(batch[row])
...
describe('addMany / removeMany (§4.3)', () => {
  ...
  test('a batch spread across archetypes is routed per source', () => {
    ...
    for (const e of plain) expect(world.has(e, Velocity)).toBe(false)
    for (const e of [...plain, ...moving]) expect(world.has(e, IsActive)).toBe(true)
...
describe('despawnMany (§4.3)', () => {
  test('every entity in the batch dies and its row is released', () => {
    ...
    for (const e of batch) expect(world.isAlive(e)).toBe(false)
...
describe('batch inputs (§4.3)', () => {
  test('any iterable of handles is a batch, which is what makes a query result one', () => {
    ...
    for (const e of [...handles(typed), ...array, ...set]) expect(world.has(e, IsActive)).toBe(true)

    world.despawnMany(set)
    for (const e of set) expect(world.isAlive(e)).toBe(false)
