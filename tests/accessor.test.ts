...
describe('the accessor follows the entity (§4.5, §10.2)', () => {
  ...
  test('when a swap-remove relocates the row', () => {
    ...
    for (let i = 0; i < 10; i++) {
      spawned.push(world.spawn(Position({ x: i })));
    }
  ...
  test('through pages appended after the accessor first resolved', () => {
    ...
    const more: Entity[] = [];
    for (let i = 1; i < 20; i++) {
      more.push(world.spawn(Position({ x: i })));
    }

    for (let i = 1; i < 20; i++) {
      expect(px.get(more[i - 1])).toBe(i);
    }
    expect(px.get(first)).toBe(0);
  ...
  test('through compact() and the pages a later spawn puts back', () => {
    ...
    const more: Entity[] = [];
    for (let i = 0; i < 6; i++) {
      more.push(world.spawn(Position({ x: 50 + i })));
    }

    expect(px.get(again)).toBe(40);
    for (let i = 0; i < 6; i++) {
      expect(px.get(more[i])).toBe(50 + i);
    }
