...
describe('sorted-static', () => {
  bench('apecs sorted (clean)', () => {
    if (stable.view.first === undefined) throw new Error('unreachable')
  })

  bench('apecs sorted (rebuild)', () => {
    rebuilding.view.rebuild()
    if (rebuilding.view.first === undefined) throw new Error('unreachable')
  })
})
...
describe('sorted-drift', () => {
  bench('apecs sorted (1% drift)', () => {
    ...
    if (drifting.view.first === undefined) throw new Error('unreachable')
  })
