...
function packedWorld(entities) {
  ...
  for (let i = 0; i < entities; i++) {
    ...
    for (let c = 0; c < comps.length; c++) addComponent(world, e, comps[c])
  }
  return { world, comps }
}
...
export const benchmarks = {
  'packed_1/raw': ({ entities }) => {
    ...
    return () => {
      ...
      const value = A.value
      for (let i = 0, n = ents.length; i < n; i++) value[ents[i]] += 1
    }
  },
  ...
  'entity_cycle/raw': ({ entities }) => {
    ...
    return () => {
      ...
      for (let i = 0; i < entities; i++) removeEntity(world, live[i])
    }
  },
  ...
  'add_remove/raw': ({ entities }) => {
    ...
    return () => {
      for (let i = 0; i < entities; i++) addComponent(world, live[i], Velocity)
      for (let i = 0; i < entities; i++) removeComponent(world, live[i], Velocity)
    }
  },
