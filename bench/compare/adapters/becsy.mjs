...
export const benchmarks = {
  ...
  'packed_5/ergonomic': async ({ entities }) => {
    ...
    class Pass extends System {
      ...
      execute() {
        for (const e of this.a.current) {
          e.write(A).value += 1;
        }
        for (const e of this.b.current) {
          e.write(B).value += 1;
        }
        for (const e of this.c.current) {
          e.write(C).value += 1;
        }
        for (const e of this.d.current) {
          e.write(D).value += 1;
        }
        for (const e of this.e.current) {
          e.write(E).value += 1;
        }
      }
    }
    ...
    await world.build((sys) => {
      for (let f = 0; f < archetypes; f++) {
        for (let i = 0; i < sizes[f]; i++) {
          sys.createEntity(Data, fragments[f]);
        }
      }
    });
    return () => world.execute();
  ...
  'entity_cycle/ergonomic': async ({ entities }) => {
    ...
    class Churn extends System {
      ...
      execute() {
        for (let i = 0; i < entities; i++) {
          this.createEntity(Position, Velocity);
        }
        for (const e of this.q.current) {
          e.delete();
        }
  ...
  'random_access/ergonomic': async ({ entities }) => {
    ...
    await world.build((sys) => {
      for (let i = 0; i < entities; i++) {
        live[i] = sys.createEntity(Position).hold();
      }
    });
    return () => world.execute();
  },
...
export async function census({ entities, archetypes, excluded }) {
...

  {
    ...
    await world.build((sys) => {
      for (let i = 0; i < entities; i++) {
        sys.createEntity(Position, Velocity);
      }
    });
    await world.execute();
  }
