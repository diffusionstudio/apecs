...
export const benchmarks = {
  'packed_1/raw': ({ entities }) => {
    const a = F(entities)
    return () => {
      for (let i = 0; i < entities; i++) a[i] += 1
    }
  },
  ...
  'packed_5/raw': ({ entities }) => {
    ...
    return () => {
      for (let c = 0; c < 5; c++) {
        ...
        for (let i = 0; i < entities; i++) column[i] += 1
      }
    }
  },
  ...
  'frag_iter/raw': ({ entities, archetypes }) => {
    ...
    return () => {
      for (let b = 0; b < blocks.length; b++) {
        ...
        for (let i = 0, n = block.length; i < n; i++) block[i] += 1
      }
    }
  },
  ...
  'entity_cycle/raw': ({ entities }) => {
    ...
    return () => {
      ...
      while (top > 0) generations[free[--top]]++
    }
  },
  ...
  'add_remove/raw': ({ entities }) => {
    ...
    return () => {
      ...
      for (let i = 0; i < entities; i++) has[i] = 0
      count = 0
    }
  },
