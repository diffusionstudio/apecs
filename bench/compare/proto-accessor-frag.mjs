...
for (let i = 0; i < N; i++) live[i] = world.spawn(Position, FILLER[i % K])
...
const bindOf = (a) => {
  ...
  return { pages: column.pages, shift: 31 - Math.clz32(column.pageSize), mask: column.pageSize - 1 }
}
...
function denseTable() {
  ...
  return (e, d) => {
    ...
    const a = entities.archetypes[id]
    let b = table[a]
    if (b === undefined) b = table[a] = bindOf(a)
    const row = entities.rows[id]
...
await run('C1 · one-entry inline cache', () => {
  for (let i = 0; i < N; i++) c1(live[order[i]], 1)
})
await run('C2 · dense table by archetype', () => {
  for (let i = 0; i < N; i++) c2(live[order[i]], 1)
})
