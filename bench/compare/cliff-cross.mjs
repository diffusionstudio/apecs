...
if (which === 'apecs') {
  ...
  for (const k of KS) {
    ...
    const fn = () => {
      for (let i = 0; i < k; i++) qs[i].each((t) => (t.value += 1))
    }
  ...
  for (const k of KS) {
    ...
    const fn = () => {
      for (let i = 0; i < k; i++) w.query(keys[i]).updateEach(([t]) => (t.value += 1))
    }
    for (let i = 0; i < 5; i++) fn()
    rows.push({ k, ns: (await measure(fn, { min_cpu_time: 700e6 })).avg / (k * N) })
  }
  ...
  for (const k of KS) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      for (const c of comps) b.addComponent(w, e, c)
    }
    ...
    const fn = () => {
      for (let i = 0; i < k; i++) {
        ...
        for (let j = 0, m = ents.length; j < m; j++) v[ents[j]] += 1
      }
    }
    for (let i = 0; i < 5; i++) fn()
    rows.push({ k, ns: (await measure(fn, { min_cpu_time: 700e6 })).avg / (k * N) })
  ...
  for (const k of KS) {
    ...
    class Pass extends System {
      ...
      execute() {
        for (let i = 0; i < comps.length; i++)
          for (const e of this.qs[i].current) e.write(comps[i]).value += 1
      }
    }
    ...
    await world.build((sys) => {
      for (let i = 0; i < N; i++) sys.createEntity(...comps)
    })
