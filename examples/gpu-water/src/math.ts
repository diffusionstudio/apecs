/** Column-major 4x4s, the layout WGSL wants, written into caller-owned arrays. */
...
export function perspective(out: Mat4, fovY: number, aspect: number, near: number, far: number) {
  ...
  out[5] = f;
  out[11] = -1;
  // Reversed-Z is not worth the extra state here; plain [0,1] depth.
  out[10] = far / (near - far);
  out[14] = (far * near) / (near - far);
  return out;
}

export function lookAt(out: Mat4, eye: ArrayLike<number>, at: ArrayLike<number>) {
  let zx = eye[0] - at[0];
  let zy = eye[1] - at[1];
  let zz = eye[2] - at[2];
  ...
  zy /= len;
  zz /= len;
  // x = normalize(cross(+Y, z)). Up is +Y and the camera never rolls, so the
  // only degenerate case is looking straight down, which the pitch clamp bars.
  let xx = zz;
  const xy = 0;
  let xz = -zx;
