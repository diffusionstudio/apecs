...
class StoreView {
  ...
  public bind(columns: Column[], page: number): Record<string, unknown> {
    ...
    for (let i = 0; i < owners.length; i++) {
      owners[i][keys[i]] = columns[slots[i]].pages[page];
    }
    return this.root;
  }
...
export class Chunk {
  ...
  public markChanged(item: TraitLike, row?: number): void {
    ...
    for (let i = 0; i < columns.length; i++) {
      ...
      if (row === undefined) {
        pages[this.page].fill(tick, 0, this.length);
      } else {
        pages[this.page][row] = tick;
      }
  ...
  public column<K extends FieldKind>(field: Field<unknown, K>): ArrayFor<K> {
    ...
    if (__DEV__) {
      assert(columns !== undefined, 'this chunk does not hold that field');
    }
...
export class Chunks implements Iterable<Chunk> {
  ...
  public next(): IteratorResult<Chunk> {
    ...
    if (__DEV__) {
      this.#chunk.flushGuards();
    }

    for (;;) {
      if (this.#page < 0) {
        let index = this.#archetype - 1;
        while (index >= 0 && this.#rowsOf(index) === 0) {
          index--;
        }
        if (index < 0) {
          return this.return();
