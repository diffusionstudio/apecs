...
export class TargetIndex {
  ...
  public listFor(target: Entity): EntityList {
    ...
    if (list === undefined) {
      this.lists.set(target, (list = new EntityList()));
    }
    return list;
  }
  ...
  public enableDepths(index: EntityIndex, tick: number): void {
    ...
    for (const [target, list] of this.lists) {
      ...
      if (alive && id < this.#at.length && this.#at[id] !== NOWHERE) {
        continue;
      }
  ...
  public forget(id: number): void {
    if (this.depths !== null && id < this.depths.length) {
      this.depths[id] = 0;
    }
  }

  #ensure(id: number): void {
    if (id < this.#at.length) {
      return;
    }
