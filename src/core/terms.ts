...
export function Or<T extends Term[]>(...operands: T): Modifier<'or', T> {
  if (__DEV__) {
    assert(operands.length > 0, 'Or() needs at least one term');
  }
...
export function isDataTerm(operand: Term): boolean {
  if (operand instanceof Trait) {
    return operand[$kind] !== 'tag';
  }

  const modifier = operand as Modifier;
  if (modifier[$term] === 'optional') {
    return isDataTerm(modifier[$terms][0]);
  }
