...
export class World {
  ...
  public spawnMany(n: number, ...items: TraitLike[]): Float64Array {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const columns = archetype.columnsOf.get(trait[$id]);
      for (let k = 0; k < n; k++) {
        initTrait(columns, first + k, trait, value, tick);
      }
      if (isExclusive(trait)) {
  ...
  public get(target: Entity | Subject, spec?: Subject | object, out?: object): any {
    if (typeof target !== 'number') {
      return this.get(this.entity, target as Trait, spec as object);
    }
    const id = entityId(target);
    if (__DEV__) {
      this.#assertAlive(target, id);
    }
  ...
  public set(target: Entity | Subject, spec?: Subject | unknown, value?: unknown): void {
    if (typeof target !== 'number') {
      return this.set(this.entity, target as Trait, spec);
    }
    const id = entityId(target);
    if (__DEV__) {
      this.#assertAlive(target, id);
    ...
    if (typeof subject !== 'function' && $index in subject) {
      ...
      this.#setTrait(target, id, traitOf(subject as TraitLike), value);
    }
  }
  ...
  #drain(): void {
    ...
    while (pending.length !== 0) {
      ...
      const id = entityId(entity);
      if (this[$entities].isAlive(id, entityGeneration(entity))) {
        this.#despawn(entity, id);
      }
    }
  }
  ...
  #clear(): void {
    for (let id = this.#nextId - 1; id >= FIRST_ENTITY_ID; id--) {
      ...
      if (generation !== 0) {
        this.#despawn(packEntity(id, generation, this[$id]), id);
      }
    }
    this.#drain();
  }
  ...
  #removing(entity: Entity, id: number): void {
    for (const [key, list] of this.#onRemove) {
      ...
      if (relation !== undefined) {
        ...
        if (held) {
          this.#dispatch(list, entity, target);
        }
        ...
        for (let i = 0; i < pairs.length; i++) {
          this.#dispatch(list, entity, pairs[i][$target]);
        }
      } else if (this.#hasTrait(id, key)) {
        this.#dispatch(list, entity, undefined);
      }
    }
  }
  ...
  #pairsIn(mask: Mask, relation: Relation, into: Pair[] | null): number {
    ...
    for (let block = 0; block < mask.length; block++) {
      ...
      while (bits !== 0) {
        ...
        if ((trait as Pair)[$relation] !== relation) {
          continue;
        }
        found++;
  ...
  #emit(map: Map<Trait, ObserverFn[]>, entity: Entity, trait: Trait, target: Entity): void {
    ...
    if (relation !== undefined) {
      ...
      if (pair !== undefined) {
        this.#fire(map.get(pair), entity, target);
      }
    } else {
      this.#fire(map.get(trait), entity, undefined);
    }
  }
  ...
  #wrote(entity: Entity, id: number, trait: Trait): void {
    if (this.#onChange.size !== 0) {
      this.#emitChange(entity, id, trait);
    }
  }
  ...
  #attached(entity: Entity, id: number, trait: Trait, target: Entity): void {
    if (this.#ticks.added.size !== 0) {
      ...
      const relation = (trait as Pair)[$relation];
      if (relation !== undefined) {
        this.#ticks.stampAdded(relation[$id], id);
      }
    }
    if (this.#onAdd.size !== 0) {
      this.#emit(this.#onAdd, entity, trait, target);
    }
  }
  ...
  #getField(id: number, field: Field): unknown {
    ...
    if (__DEV__) {
      this.#assertReadable(trait, columns);
    }
  ...
  #destination(from: Archetype, items: readonly TraitLike[], start: number): Archetype {
    ...
    for (let i = start; i < items.length; i++) {
      const trait = traitOf(items[i]);
      if (__DEV__) {
        this.#assertAddable(trait, targetOf(items[i]));
      }
      const relation = (trait as Pair)[$relation];
      if (relation !== undefined) {
        const bit = traits.register(relation);
        if (!maskHas(to.mask, bit)) {
          to = graph.edgeAdd(to, bit);
        }
      }
      const local = traits.register(trait);
      if (!maskHas(to.mask, local)) {
        to = graph.edgeAdd(to, local);
      }
    }
    return to;
  }
  ...
  #add(entity: Entity, id: number, items: readonly TraitLike[], start: number): void {
    ...
    if (to !== from && this.#boundaries.length !== 0) {
      this.#crossed(entity, from, to);
    }
  }
  ...
  #link(
    ...
    if (state.depths !== null) {
      state.reroot(entity, state.depthOf(target) + 1, tick);
    }
  }
  ...
  #retarget(
    ...
    if (moved) {
      if (this.#onRemove.size !== 0) {
        this.#emit(this.#onRemove, entity, relation, previous);
      }
      this.#unlink(relation, entity, id, previous, tick);
    }
    if (value !== undefined) {
      initTrait(columns, row, relation, value, tick);
    }
    if (moved) {
      this.#link(relation, entity, id, columns, row, target, tick);
    } else if (value !== undefined) {
      column.set(row, target);
      column.stamp(row, tick);
    }
  ...
  #remove(entity: Entity, id: number, items: readonly TraitLike[], start: number): void {
    ...
    if (this.#onRemove.size !== 0) {
      for (let i = start; i < items.length; i++) {
        this.#leaving(entity, id, items[i]);
      }
    }
    ...
    for (let i = start; i < items.length; i++) {
      ...
      if (isExclusive(trait)) {
        ...
        const target = targetOf(item);
        if (typeof target === 'number' && target !== NULL_ENTITY && target !== current) {
          continue;
        }
  ...
  #leaving(entity: Entity, id: number, item: TraitLike): void {
    ...
    } else if (this.#hasTrait(id, trait)) {
      this.#emit(this.#onRemove, entity, trait, NULL_ENTITY);
    }
  }
  ...
  #move(id: number, entity: Entity, from: Archetype, to: Archetype): number {
    ...
    if (__DEV__) {
      this.#iteration.assertRemovable(from, source);
    }
    ...
    if (moved !== NULL_ENTITY) {
      entities.rows[entityId(moved)] = source;
    }
  ...
  #release(id: number): void {
    ...
    if (__DEV__) {
      this.#iteration.assertRemovable(archetype, row);
    }
    ...
    for (let i = 0; i < states.length; i++) {
      ...
      if (!maskHas(archetype.mask, state.local)) {
        continue;
      }
    ...
    if (moved !== NULL_ENTITY) {
      entities.rows[entityId(moved)] = row;
    }
  ...
  #resolveTargets(entity: Entity): void {
    ...
    for (let s = 0; s < states.length; s++) {
      ...
      if (policy === 'orphan') {
        ...
        if (state.depths !== null) {
          for (let i = 0; i < list.length; i++) {
            state.reroot(list.items[i] as Entity, 1, tick);
          }
        }
        continue;
      }
      if (policy === 'despawn') {
        for (let i = 0; i < list.length; i++) {
          this.#pending.push(list.items[i]);
        }
      } else {
    ...
    for (let p = 0; p < pairs.length; p++) {
      ...
      if (local >= 0) {
        ...
        for (let a = archetypes.length - 1; a >= 0; a--) {
          ...
          if (!maskHas(archetype.mask, local)) {
            continue;
          }
          if (policy === 'despawn') {
  ...
  #assertAddable(trait: Trait, target: Entity | '*'): void {
    if (isRelation(trait)) {
      ...
      if (pair[$relation] !== undefined) {
        this.#assertAlive(pair[$target], entityId(pair[$target]));
      }
    }
  }
