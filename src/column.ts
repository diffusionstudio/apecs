...
export class Column {
  ...
  public constructor(field: Field, pageSize: number) {
    ...
    if ((field[$trait] as Trait | null)?.[$options].track) {
      this.ticks = [];
    }
  }
  ...
  public ensure(rows: number): void {
    ...
    if (ticks !== null) {
      while (ticks.length < needed) {
        ticks.push(new Uint32Array(pageSize));
      }
    }
  }
  ...
  public track(): void {
    ...
    for (let i = 0; i < this.pages.length; i++) {
      ticks.push(new Uint32Array(this.pageSize));
    }
    this.ticks = ticks;
  }
  ...
  public swapRemove(row: number, last: number): void {
    if (row !== last) {
      ...
      if (this.ticks !== null) {
        this.ticks[row >>> this.shift][row & this.mask] = this.tickOf(last);
      }
    }
    if (this.boxed) {
      (this.pages[last >>> this.shift] as unknown[])[last & this.mask] = undefined;
    }
  }
