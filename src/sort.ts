...
export function sortByKey(order: number[], keys: Float64Array): void {
  ...
  if (n < 2) return
  if (scratch.length < n >>> 1) scratch = new Uint32Array(n)
  ...
  for (let lo = 0; lo < n;) {
    ...
    if (hi < n) {
      ...
      if (key < previous) {
        ...
        previous = key
        while (++hi < n && (key = keys[order[hi]]) < previous) previous = key
    ...
    if (hi - lo < MIN_RUN) hi = extend(order, keys, lo, hi, Math.min(lo + MIN_RUN, n))
    runs.push(hi)
    lo = hi
  }
  ...
  while (runs.length > 2) {
    ...
    if (r + 1 < runs.length) runs[w++] = runs[r + 1]
    runs.length = w
  }
}
...
function extend(order: number[], keys: Float64Array, lo: number, hi: number, end: number): number {
  for (; hi < end; hi++) {
    ...
    while (left < right) {
      ...
      if (key < keys[order[mid]]) right = mid
      else left = mid + 1
    }
    for (let i = hi; i > left; i--) order[i] = order[i - 1]
    order[left] = item
  }
  return end
}
...
function merge(order: number[], keys: Float64Array, lo: number, mid: number, hi: number): void {
  if (keys[order[mid - 1]] <= keys[order[mid]]) return
  const left = mid - lo
  const right = hi - mid

  if (left <= right) {
    for (let i = 0; i < left; i++) scratch[i] = order[lo + i]
    let i = 0
    let j = mid
    let k = lo
    while (i < left && j < hi) {
      if (keys[order[j]] < keys[scratch[i]]) order[k++] = order[j++]
      else order[k++] = scratch[i++]
    }
    while (i < left) order[k++] = scratch[i++]
  } else {
    for (let j = 0; j < right; j++) scratch[j] = order[mid + j]
    let i = mid - 1
    let j = right - 1
    let k = hi - 1
    while (i >= lo && j >= 0) {
      if (keys[scratch[j]] >= keys[order[i]]) order[k--] = scratch[j--]
      else order[k--] = order[i--]
    }
    while (j >= 0) order[k--] = scratch[j--]
  }
}
