...
export class Ticks {
  ...
  public trackAdded(traitId: number): void {
    if (!this.added.has(traitId)) {
      this.added.set(traitId, EMPTY_TABLE);
    }
  }
  ...
  public logRemoved(entity: Entity, traitId: number): void {
    ...
    if (this.#removedTicks.length >= this.#expireAt) {
      this.#expire();
    }
  ...
  public collectRemoved(traitId: number, after: number, into: Set<number>): void {
    ...
    for (let i = 0; i < traits.length; i++) {
      if (traits[i] === traitId && ticks[i] > after) {
        into.add(this.#removedEntities[i]);
      }
    }
  }
  ...
  #expire(): void {
    ...
    let cut = 0;
    while (cut < ticks.length && ticks[cut] < horizon) {
      cut++;
    }
    if (cut > 0) {
